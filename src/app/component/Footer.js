    ...
    return (
        <footer className="overflow-clip bg-dark-primary min-w-full !max-w-full min-h-[10] flex flex-col md:flex-row md:gap-10 md:pl-10 py-7 px-6 lg:pl-16">
            <section className="mincomp flex flex-col md:max-w-[35%] md:min-w-[35%] md:justify-center ">
                <Link href="/" prefetch={false} className={`${frijole.className} text-3xl`}>WOLFMAN</Link>
                <p className="text-sm pl-1">© 2024 WOLFMAN. All rights reserved.</p>
                <p className="text-sm pt-1.5 pl-1">Made with <i className="fa-solid fa-heart"></i> By <Link href="//github.com/PencariKode" prefetch={false} className="font-bold link-underline link-underline-black">Panji Depari</Link></p>
            </section>
                            ...
                            {link.map((item, index) => (
                                <div key={index} className="mincomp flex flex-col gap-1  xs:max-w-[50%] xs:min-w-[50%]">
                                    {item.map((subitem, index) => (
                                        <Link key={index} prefetch={false} href={subitem.link} className="text-sm link-underline link-underline-black">{subitem.nama}</Link>
                                    ))}
                                </div>
                            ))}
                        </div>
